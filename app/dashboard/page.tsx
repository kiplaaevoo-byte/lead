...
export default function Dashboard(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const loc = pol.county || pol.constituency || "Kenya"
      const name = pol.name || "Mheshimiwa"

      if(m && m.length>0) setMentions(m)
      else setMentions([
        { id:1, platform:"X", text:`${name} is doing great work in ${loc}! We appreciate the development.`, sentiment:"positive", created_at:new Date().toISOString() },
        { id:2, platform:"Facebook", text:`People complaining about roads in ${loc}. ${name} should act fast.`, sentiment:"negative", created_at:new Date().toISOString() },
        { id:3, platform:"TikTok", text:`Youths praising ${name} bursary program in ${loc}`, sentiment:"positive", created_at:new Date().toISOString() },
      ])
      setLoading(false)
    })()
  },[])
  ...
  const total = mentions.length || 3
  const score = total? Math.round((positive/total)*100) : 68
  const isBasic = politician?.plan === 'basic'
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <main className="p-6 md:p-8 max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-6 rounded-[24px] bg-[#101010] border border-white/10"><div className="text-[11px] tracking-widest text-white/40">TOTAL MENTIONS (24H)</div><div className="mt-2 text-[36px] font-black">{isBasic? total : total*47}</div><div className="mt-2 text-xs text-[#00ff66]">● Live scanning every 15 min</div></div>
          ...
          <div className="p-6 rounded-[24px] bg-[#00ff66] text-black"><div className="text-[11px] tracking-widest text-black/50">PRIVACY STATUS</div><div className="mt-2 text-[22px] font-black leading-[0.9]">Your data is 100% private</div><div className="mt-2 text-xs text-black/60">Plan: {politician?.plan} • No other leader can see</div></div>
        </div>

        <div className="mt-8 grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-6">
          <div className="rounded-[24px] bg-[#101010] border border-white/10 p-6 relative overflow-hidden">
            ...
            {isBasic && (
              <div className="absolute inset-0 z-20 bg-black/70 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center mt-[60px] rounded-b-[24px]">
                <div className="text-2xl">🔒</div>
                <h3 className="mt-2 font-black text-lg">Upgrade to see mentions</h3>
                <p className="mt-1 text-sm text-white/60 max-w-[280px]">You are on FREE BASIC plan. You can see numbers only. Upgrade to STANDARD KES 5,000/mo to see full live feed.</p>
                <button onClick={()=>router.push("/pricing")} className="mt-4 px-6 py-3 rounded-full bg-[#00ff66] text-black font-bold text-sm">Upgrade to Standard - KES 5k</button>
              </div>
            )}

            <div className={`mt-6 space-y-3 ${isBasic? 'blur-[8px] pointer-events-none' : ''}`}>
              {mentions.map((m:any)=>(
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[24px] bg-[#101010] border border-white/10 p-6"><h3 className="font-bold text-sm">Quick Actions</h3><div className="mt-4 grid grid-cols-1 gap-2"><button className="w-full py-3 rounded-full bg-white text-black text-sm font-bold">Download Private PDF Report</button><button className="w-full py-3 rounded-full bg-white/10 border border-white/10 text-sm">Clear Negative Alerts</button>{isBasic? <button onClick={()=>router.push("/pricing")} className="w-full py-3 rounded-full bg-[#00ff66] text-black text-sm font-bold">Upgrade to Real-Time Alerts (KES 5k/mo)</button> : <button className="w-full py-3 rounded-full bg-[#00ff66]/10 border border-[#00ff66]/20 text-[#00ff66] text-sm">You are on {politician?.plan?.toUpperCase()}</button>}</div></div>

            <div className="rounded-[24px] bg-[#00ff66]/10 border border-[#00ff66]/20 p-5"><div className="text-xs text-[#00ff66] font-bold">🔒 PRIVATE BY DESIGN</div><p className="mt-2 text-xs text-white/60 leading-relaxed">Dashboard isolated via Supabase RLS. Plan: {politician?.plan}. No public API. Only you (user_id = {user?.id?.slice(0,8)}...) can read your row.</p></div>
          </div>
        </div>
      </main>
    </div>
