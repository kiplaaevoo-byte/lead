...
export default function AuthPage(){
  ...
  const [form,setForm]=useState({ name:"", email:"", phone:"", position:"MP", county:"Nairobi", password:"", confirm:"", keywords:"" })
  ...
  const validate = ()=>{
    ...
    if(form.phone && form.phone.length<10) return "Enter valid phone (07...)"
  ...
  const handleSignup = async ()=>{
    ...
    try{
      ...
      if(data.user){
        ...
        const av = `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(form.name)}&backgroundColor=00ff66,1a1a1a&chars=2`
        setAvatarUrl(av)
        // create politician profile
        const { error: pError } = await supabase.from("politicians").insert({
          name: form.name,
          user_id: data.user.id,
          keywords: [form.name, form.name.split(" ").pop(), form.county,...(form.keywords? form.keywords.split(",").map(k=>k.trim()): [])].filter(Boolean),
        ...
        setShowSuccess(true)
        setTimeout(()=>{ setShowSuccess(false); setMode("login"); router.push("/login") }, 3500)
      }
        ...
        <div className="max-w-[460px] w-full mx-auto">
          ...
          <div className="mt-8 space-y-4">
            {mode==="signup" && <>
              ...
              <input value={form.keywords} onChange={e=>setForm({...form,keywords:e.target.value})} placeholder="Keywords to track (comma separated) e.g. Ruto, Zakayo" className="w-full px-5 py-3.5 rounded-2xl bg-white/[0.06] border border-white/10 outline-none" />
            </>}
